...
//        $the_artwork = mysqli_fetch_assoc($sql_artworkID_result);
    ...
    $sql ="UPDATE users SET balance=_balance WHERE userID=_userID";
    $sql = preg_replace("/_userID/",$current_user['userID'],$sql);
    $sql = preg_replace("/_balance/",$current_user['balance'] - $price_sum,$sql);
    $mysql->query($sql);
    $sql = "DELETE FROM carts WHERE userID=_userID";
    $sql = preg_replace("/_userID/",$current_user['userID'],$sql);
    $sql_result = mysqli_query($mysql,$sql);
    echo "Success";
}
